...
LcdDisplay::LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height)
    ...
    esp_timer_create_args_t speaking_timer_args = {
        .callback = [](void* arg) {
            LcdDisplay* display = static_cast<LcdDisplay*>(arg);
            display->UpdateSpeakingAnimation();
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "speaking_timer",
    ...
    esp_timer_create(&speaking_timer_args, &speaking_timer_);
}
...
void LcdDisplay::ApplyEmojiScaleLocked(const lv_img_dsc_t* img_dsc) {
    if (emoji_image_ == nullptr || img_dsc == nullptr || img_dsc->header.w <= 0 || img_dsc->header.h <= 0) {
        return;
    }

    int max_dimension = std::max<int>(img_dsc->header.w, img_dsc->header.h);
    int target_size = std::min(width_, height_) * kEmojiFillPercent / 100;
    int32_t scale = std::max<int32_t>(32, 256 * target_size / max_dimension);
    lv_image_set_scale(emoji_image_, scale);
}

const LvglImage* LcdDisplay::FindThemeEmojiImageLocked(const char* emotion) {
    if (current_theme_ == nullptr || emotion == nullptr || emotion[0] == '\0') {
        return nullptr;
    }
    ...
    return emoji_collection->FindEmojiImage(emotion);
}
...
void LcdDisplay::UpdateEmojiVisualLocked(const char* emotion) {
    auto image = FindThemeEmojiImageLocked(emotion);
    if (image == nullptr) {
        const char* utf8 = font_awesome_get_utf8(current_emotion_.c_str());
    ...
    if (image->IsGif()) {
        ...
        if (gif_controller_->IsLoaded()) {
            gif_controller_->SetFrameCallback([this]() {
                ...
                ApplyEmojiScaleLocked(gif_controller_->image_dsc());
            });
            lv_image_set_src(emoji_image_, gif_controller_->image_dsc());
            ApplyEmojiScaleLocked(gif_controller_->image_dsc());
    ...
    ApplyEmojiScaleLocked(image->image_dsc());
    lv_obj_add_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_remove_flag(emoji_image_, LV_OBJ_FLAG_HIDDEN);
}

void LcdDisplay::UpdateSpeakingAnimation() {
    if (!setup_ui_called_ || emoji_image_ == nullptr || current_emotion_.empty()) {
        return;
    }
    ...
    if (!is_speaking || !current_emotion_has_speaking_variant_ || gif_controller_) {
        ...
        UpdateEmojiVisualLocked(current_emotion_.c_str());
        return;
    }

    speaking_animation_frame_ = !speaking_animation_frame_;
    std::string emotion_name = current_emotion_;
    if (speaking_animation_frame_) {
        emotion_name += kSpeakingEmotionSuffix;
    }
    UpdateEmojiVisualLocked(emotion_name.c_str());
}
...
void LcdDisplay::SetupUI() {
    ...
    if (speaking_timer_ != nullptr && !esp_timer_is_active(speaking_timer_)) {
        ESP_ERROR_CHECK(esp_timer_start_periodic(speaking_timer_, kSpeakingAnimationIntervalUs));
    }
}
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    current_emotion_has_speaking_variant_ = FindThemeEmojiImageLocked(speaking_emotion.c_str()) != nullptr;
    auto base_image = FindThemeEmojiImageLocked(current_emotion_.c_str());
    ...
    uint32_t child_count = lv_obj_get_child_cnt(content_);
    if (strcmp(current_emotion_.c_str(), "neutral") == 0 && child_count > 0) {
